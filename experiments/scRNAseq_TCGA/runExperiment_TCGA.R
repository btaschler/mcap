...
runExperiment_TCGA <- function(fid_data, methods_arr, dir_out = NULL, 
  ...
  for(ds_par in seq_along(fid_data)){   #HACK because parallel does not work !!
    ...
    for(g in seq(n_grid)){ 
      ...
      curr_xx <- data$xx[[g]]
      curr_xx <- as.matrix(curr_xx[2:ncol(curr_xx)])  #hack for TCGA data (exclude sample ID)
      ...
      for(m in methods_arr){ 
        ...
        fid_out <- file.path(dir_out, 'TCGA_')
